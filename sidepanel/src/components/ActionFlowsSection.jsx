...
export function ActionFlowsSection({ flows, onCreate, onEdit, onDelete, onRun, t }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <section className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-brand">
        <div className="space-y-1">
          <h2 className="text-lg font-semibold text-slate-900">{t('flow.library.heading')}</h2>
          <p className="text-xs text-slate-500">{t('flow.library.description')}</p>
        </div>
        <button
          type="button"
          ...
          <div className="grid auto-rows-fr grid-cols-[repeat(auto-fill,minmax(180px,1fr))] gap-4">
            {sorted.map((flow) => (
              <article
                ...
                role="button"
                tabIndex={0}
                className="flex h-full cursor-pointer flex-col justify-between rounded-2xl border border-slate-200 bg-white p-4 shadow-brand transition hover:cursor-pointer hover:border-blue-200 hover:shadow-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-200"
                onClick={() => onEdit?.(flow)}
                onKeyDown={(event) => {
              ...
              >
                <div className="space-y-1">
                  ...
                  <p className="line-clamp-2 text-xs text-slate-500">
                    {flow.description || t('flow.library.noDescription')}
                  </p>
                ...
                <div className="mt-3 flex flex-wrap gap-1.5 text-xs">
                  <button
                    type="button"
                    className="btn-secondary inline-flex h-7 w-7 items-center justify-center p-0 text-xs"
                    onClick={(event) => {
