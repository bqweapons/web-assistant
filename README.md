...
### Overview
Page Augmentor is a Manifest V3 Chrome extension that lets you inject custom buttons or links into any webpage and manage them from a dedicated side panel. Each item is stored per URL in `chrome.storage.local`, so elements automatically reappear whenever you revisit the page.
...
- **In-page bubble editor** – Choose between button or link, set the label and optional destination URL, select the insertion position (append, prepend, before, after), and fine-tune colors, typography, spacing, and radius with a live preview before saving.
...
3. Configure the element in the bubble editor: select type, text, optional URL, insertion position, and style settings. Confirm to save.
...
- `storage` – persist button/link metadata per page.
...
- Nested `iframe` elements are not currently supported.
...
### 概要
Page Augmentor は Manifest V3 ベースの Chrome 拡張機能で、任意の Web ページにカスタムのボタンやリンクを追加し、専用のサイドパネルから一元管理できます。各アイテムは URL 単位で `chrome.storage.local` に保存され、ページを再訪すると自動的に復元されます。
...
- **ページ内バブルエディター** – ボタン／リンクの切り替え、ラベルと任意の遷移先 URL の設定、挿入位置（末尾、先頭、直前、直後）の選択、色・タイポグラフィ・余白・角丸などのスタイル微調整をライブプレビュー付きで行えます。
...
3. バブルエディターでタイプ、テキスト、任意の URL、挿入位置、スタイルを設定し、保存します。
...
- `storage` – ページごとのボタン／リンク設定を永続化するため。
...
- 入れ子になった `iframe` への要素注入には対応していません。
...
### 概述
Page Augmentor 是一款基于 Manifest V3 的 Chrome 扩展，可在任意网页注入自定义按钮或链接，并通过扩展侧边栏集中管理。所有注入元素都会按照页面 URL 保存在 `chrome.storage.local` 中，重新访问时会自动恢复。

### 功能亮点
- **侧边栏一体化视图**：在当前页面的「元素管理」与汇总全部页面的「全局总览」之间自由切换，并实时显示统计信息。
- **可视化拾取器**：直接在页面上高亮 DOM 节点，自动生成 CSS 选择器，并立即弹出编辑气泡完成配置。
- **页面内气泡编辑器**：在保存前即可切换按钮/链接类型，设置文本和可选跳转地址，选择插入位置（追加、前置、前插、后插），并实时预览颜色、排版、间距、圆角等样式调整。
...
3. 在气泡编辑器中设置类型、文本、可选 URL、插入位置与样式，并确认保存。
